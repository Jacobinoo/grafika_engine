...
private:
    ...
    float moveSpeed = 2.0f; /**< Predkosc chodzenia */
...
public:
    ...
    Player(float startX, float startY) : GameObject("Player", startX, startY) {
        ...
        auto animPtr = std::make_unique<AnimatedSpriteComponent>(
            AssetManager::GetImage("Player"), 16, 16
        );
        anim = animPtr.get();
    ...
    void Update() override {
        ...
        if (Input::IsActionHeld("PlayerMoveUp"))    velocity.y = -moveSpeed;
        if (Input::IsActionHeld("PlayerMoveDown"))  velocity.y = moveSpeed;
        if (Input::IsActionHeld("PlayerMoveLeft"))  velocity.x = -moveSpeed;
        if (Input::IsActionHeld("PlayerMoveRight")) velocity.x = moveSpeed;
